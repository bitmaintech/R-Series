...
#if defined(USE_BMSC) || defined(USE_ANTROUTER)
...
#ifdef USE_BMSC
#define DRIVER_drv DRIVER_bmsc
#endif

#ifdef USE_ANTROUTER
#define DRIVER_drv DRIVER_antrouter
#endif
...
static bool usb_check_device(struct device_drv *drv, struct libusb_device *dev, struct usb_find_devices *look)
{
	...
	err = libusb_get_device_descriptor(dev, &desc);
	if (err) {
		applog(LOG_ERR, "USB check device: Failed to get descriptor, err %d", err);
		return false;
	}
