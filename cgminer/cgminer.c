...
#ifdef USE_ANTROUTER
static char *set_antrouter_options(const char *arg)
{
	opt_set_charp(arg, &opt_antrouter_options);

	return NULL;
}

static char *set_antrouter_volt(const char *arg)
{
	opt_set_charp(arg, &opt_antrouter_volt);
...
#endif
#ifdef USE_ANTROUTER
	OPT_WITH_ARG("--antrouter-options",
		     set_antrouter_options, NULL, NULL,
		     opt_hidden),
...
void _discard_work(struct work *work)
{
	if (!work->clone && !work->rolls && !work->mined) {
		...
		total_discarded++;
		applog(LOG_DEBUG, "Discarded work %d",total_discarded);
	} else
...
void submit_nonce_2(struct work *work)
{
	...
	if(found_blocks != tmp){
		...
		swap256(bedata, work_out->data + 4);
		__bin2hex(hash, bedata, 32);
		//applog(LOG_ERR,"%s",hash);	
			cgtime(&tv);
		...
		g_log_block = fopen("/config/getblk", "a+");
			
		if(g_log_block) {
			fwrite(datetime, strlen(datetime), 1, g_log_block);
			fwrite(hash, strlen(hash), 1, g_log_block);
			fwrite("\n", 1, 1, g_log_block);
			fflush(g_log_block);
			fclose(g_log_block);
			tmp = found_blocks;
